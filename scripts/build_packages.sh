...
pushd "$BASE_DIR" > /dev/null
    echo "Executing packages build steps for Raider..."
    ...
    if [ $rc -eq 0 ]; then
        echo "Success: Done executing packages build steps for Raider"
    else
        echo "Error: Failed executing packages build steps for Raider"
    fi
popd > /dev/null
